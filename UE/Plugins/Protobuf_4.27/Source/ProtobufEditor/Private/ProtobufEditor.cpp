...
FString FProtobufEditorModule::GetProtoSourceFilePath() const
{
	// Project Source path.
	// FString ProtoPath= FPaths::GameSourceDir();
	
	//Plugin Source path.
	FString MocapDataStreamPluginPath = IPluginManager::Get().FindPlugin(TEXT("Protobuf"))->GetBaseDir();
	MocapDataStreamPluginPath.Append(TEXT(""));
	FString ProtoPath = FPaths::Combine(MocapDataStreamPluginPath, TEXT("Source/Protobuf/Protocol"));
	return ProtoPath;
}
...
void FProtobufEditorModule::ExecuteProtoc(const FString& pProtocExe, const FString& pProtoFilePath,FString pOutPath,FString pLang,bool pWaitProcExit)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("ProtoFile = %s"), *ProtoFile);
...
void FProtobufEditorModule::RenameAllProtoCodeFiles(const FString& PbCcPath)
{
	if (FPaths::DirectoryExists(PbCcPath))
	{
		...
		for (const auto& File : AllFiles)
		{
			...
			if (!NewFile.IsEmpty())
			{
				NewFile = NewFile.Replace(TEXT("Protocol"), TEXT("Private/ProtoGen"),ESearchCase::CaseSensitive);
				IFileManager::Get().Move(*NewFile, *File);
			}
		}

		//01 处理.h
		IFileManager::Get().FindFilesRecursive(AllFiles, *PbCcPath, TEXT("*.pb.h"), true, false, true);
		for (const auto& File : AllFiles)
		{
			FString NewFile=File;
			if (!NewFile.IsEmpty())
